...
// 引入班级小仓库
import useClassStore from "../../../store/modules/types/class";
let classStore = useClassStore();
// 引入学校数据类型
import type { schoolInfoData } from "../../../api/school/type";
// 引入学校小仓库
import useSchoolStore from "../../../store/modules/types/school";
let schoolStore = useSchoolStore();
// 引入专业数据类型
import type { majorInfoData } from "../../../api/major/type";
// 引入专业小仓库
import useMajorStore from "../../../store/modules/types/major";
...
const updateData = (formEl: FormInstance | undefined) => {
  ...
  formEl.validate(async (valid) => {
    if (valid) {
      ...
      studentStore.updateStudent(updateStudent.value).then((res) => {
...
//富文本操作
// npm install --save  @tinymce/tinymce-vue -S    安装插件
import Editor from "@tinymce/tinymce-vue";
const tiny = reactive({
  apiKey: "zyzjz6enuumn56l9m279q4c7ygw7vodf89w3efhsu7sijn1b",
  init: {
...
const validateIdNumber = (rule: any, value: any, callback: any) => {
  value = value.trim();
  const idNumberReg =
    /^(^[1-9]\d{5}(18|19|([23]\d))\d{2}((0[1-9])|(10|11|12))(([0-2][1-9])|10|20|30|31)\d{3}[0-9Xx]$)|(^[1-9]\d{5}\d{2}((0[1-9])|(10|11|12))(([0-2][1-9])|10|20|30|31)\d{2}[Xx]$)$/;
  if (value === "") {
...
const rules = reactive<FormRules>({
  ...
  manage_id: [{ required: true, message: "请选择学管", trigger: "blur" }],
});
...
import useStudentStore from "../../../store/modules/types/student";
