...
const addData = (formEl: FormInstance | undefined) => {
  ...
  formEl.validate(async (valid) => {
    if (valid) {
      schoolStore.addSchool(school.value as schoolInfoData).then((res) => {
...
const updateData = (formEl: FormInstance | undefined) => {
  ...
  formEl.validate(async (valid) => {
    if (valid) {
      schoolStore.updateSchool(school.value as schoolInfoData).then((res) => {
        schoolFormVisible.value = false;
        getSchoolList();
      });
    } else {
...
const handleDelete = (_id: string) => {
  ElMessageBox.confirm("确定删除此条数据吗?", "Warning", {
    ...
    schoolStore.deleteSchool(_id).then((res) => {
      getSchoolList();
