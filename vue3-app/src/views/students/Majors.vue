...
const addData = (formEl: FormInstance | undefined) => {
  ...
  formEl.validate((valid) => {
    if (valid) {
      majorStore.addMajor(major.value as majorInfoData).then((res) => {
...
const updateData = (formEl: FormInstance | undefined) => {
  ...
  formEl.validate((valid) => {
    if (valid) {
      majorStore.updateMajor(major.value as majorInfoData).then((res) => {
        majorFormVisible.value = false;
        getMajorList();
      });
    } else {
...
const handleDelete = (_id: string) => {
  ElMessageBox.confirm("你确定删除此条数据吗?", "Warning", {
